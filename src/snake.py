...
class Snake:
    ...
    def __init__(self, length, step, snake_n=1):
        self.x = []
        self.y = []
        self.length = length
        ...
        for i in range(length):
            x_n = random.randint(0, Game.MAX_COORD)
            y_n = random.randint(0, Game.MAX_COORD)
            self.x.append(self.STEP * self.MUL * (x_n - i))
            self.y.append(self.STEP * self.MUL * y_n)

        self.NETWORK = Network(mut_rate=0.7)
        self.snakeNumber = snake_n
    ...
    def update(self):
        ...
        if self.updateCount >= self.updateCountMax:
            self.NETWORK.set_input_values([self.inputX, self.inputY, self.wallDistance])
    ...
    def setupNetwork(self):
        ...
        self.NETWORK.set_input_values([Game.Game.WIDTH, Game.Game.HEIGHT, Game.Game.WIDTH - self.STEP])
